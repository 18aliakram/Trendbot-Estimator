...
// Create default contractor user if database is empty
...
if (users.length === 0) {
  ...
  console.log('Seeded default user account: contractor@sanjosebuild.com / admin123');
}
...
app.post('/api/projects/:id/upload', authenticateToken, upload.single('plan'), async (req, res) => {
  ...
  try {
    ...
    // Call AI Service (handles progress output internally)
    ...
    result.items.forEach(item => {
      ...
      db.insert('takeoff_items', {
        ...
        notes: item.notes || '',
        status: item.confidence < 0.90 ? 'Review Required' : 'AI Detected', // Status: AI Detected, Review Required, Contractor Approved, Contractor Edited
        unitPrice: matchedPrice ? matchedPrice.price : 0, // Default to price if found, else 0 (missing pricing)
    ...
    res.status(500).json({ 
      message: 'Plan analysis failed. Please try again. If the issue persists, check if the PDF is password-protected or unreadable.' 
    });
  }
});
...
function calculateDeterministicEstimate(takeoffItems, settings) {
  ...
  takeoffItems.forEach(item => {
    ...
    const wasteMod = 1 + (item.wastePercent !== undefined ? item.wastePercent : 0) / 100;
    ...
    // Accumulate by type if possible, default to material
  ...
  // Calculate waste factor on Materials only
...
app.get('/api/projects/:id/estimate', authenticateToken, (req, res) => {
  ...
  // Upsert actual estimate entry in the db
...
app.get('/api/audit-logs', authenticateToken, (req, res) => {
  ...
  res.json(logs.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)).slice(0, 50)); // Last 50 actions
});
...
app.listen(PORT, () => {
  console.log(`BuildEstimate AI API Server running on port ${PORT}`);
});
